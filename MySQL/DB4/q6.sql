...
SELECT REQUEST_ID, STUDENT_NAME FROM CLASS_REQUEST c JOIN STUDENT b
USING(STUDENT_ID) ORDER BY c.REQUEST_ID;

-- 2. CLASS_REQUEST 테이블에 대해 JOIN절과 USING을 이용해 CLASS테이블과 결합하여
-- 신청번호(REQUEST_ID)와 과목이름(CLASS_NAME)을 조회하는 쿼리를 작성해보세요.
SELECT REQUEST_ID, CLASS_ID FROM CLASS_REQUEST c JOIN CLASS c2
USING(CLASS_ID) ORDER BY c.REQUEST_ID;
...
SELECT REQUEST_ID, STUDENT_NAME, CLASS_ID FROM CLASS_REQUEST c 
JOIN STUDENT b USING(STUDENT_ID)  
JOIN CLASS c2 USING(CLASS_ID)
